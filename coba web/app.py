...
def download_audio(video_url, socketio):
    ydl_opts = {
        ...
        'noplaylist': True,
        'ffmpeg_location': ffmpeg_path,  # Menambahkan lokasi ffmpeg
    }
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        info = ydl.extract_info(video_url, download=False)  # Mendapatkan informasi video
        title = info.get('title', 'Unknown Title')
        ydl.download([video_url])
        socketio.emit('update', f"Downloaded and converted berhasil: {title}")
...
def handle_start_download(playlist_url):
    ...
    for video_url in playlist.video_urls:
        try:
            ...
            socketio.emit('update', f"Error downloading video: {e}")
